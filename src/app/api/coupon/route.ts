...
export async function GET(req: NextRequest) {
  ...
  console.log('User info in coupon API:', JSON.stringify(user, null, 2));
  ...
  console.log('Is Admin (coupon):', finalIsAdmin);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const user = await authenticateToken(token);
    console.log('User info in coupon POST API:', JSON.stringify(user, null, 2));
    
    // Xác định quyền admin
    const isAdmin = user?.isAdmin === true;
    const adminFallback = 
      // Kiểm tra permissions
      user?.permissions?.some((item: any) => 
        item.permission?.permission === "create" || item.permission?.permission === "update"
      ) ||
      // Kiểm tra username
      user?.username === "admin" ||
      // Kiểm tra roleId
    ...
    const finalIsAdmin = isAdmin || adminFallback;
    console.log('Is Admin (coupon POST):', finalIsAdmin);
    ...
    const notificationCustomers = customers.map(async (item: { customer_id: number }) => {
      const messages =
